...
export function match<T, E>(
...
): MatchFunc<T, E | GalileoGeneratedDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<
        T,
        E | GalileoGeneratedDefaultError | ResponseValidationError
      >,
      raw: unknown,
    ]
  > {
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new GalileoGeneratedDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new GalileoGeneratedDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
