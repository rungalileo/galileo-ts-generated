 ...
 * Update Stage
 */
export function protectUpdateStageProjectsProjectIdStagesStageIdPost(
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations
          .UpdateStageProjectsProjectIdStagesStageIdPostRequest$outboundSchema,
        value,
      ),
  ...
  const pathParams = {
    ...
    stage_id: encodeSimple("stage_id", payload.stage_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/projects/{project_id}/stages/{stage_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.StageDB$inboundSchema),
